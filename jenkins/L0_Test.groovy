...
// Recognised failure patterns are tagged with scope=SLURM or BOTH in PATTERN_CATALOG below.
...
// or BOTH in PATTERN_CATALOG.
K8S_INFRA_RETRY_MAX = 2

// ============================================================================
// Typed-exception hierarchy for infra/test failure classification.
//
// `classify(ex, scope)` (defined below) returns one of these. Producers may
// throw them directly; `classify` short-circuits on instanceof and returns
...
// contaminate. Origins for each entry: BOTH = Jenkins remoting plugin /
// durable-task plugin / kubelet (shared platform infra); SLURM =
// SLURM CLI + SSH (e.g. "DUE TO TIME LIMIT", "No route to host"); K8S =
// kubelet kubernetes-specific events (ImagePullBackOff, OOMKilled, etc.).
// All entries are third-party message strings; producers in our own code
// throw typed exceptions directly (see `<typed:...>` markers in detectedPattern).
PATTERN_CATALOG = [
...
// a fresh pod on each retry rather than retrying the test body inside a
// dying pod.
//
